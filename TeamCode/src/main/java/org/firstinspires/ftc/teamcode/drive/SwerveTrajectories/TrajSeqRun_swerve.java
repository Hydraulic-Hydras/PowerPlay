...
@Config
public class TrajSeqRun_swerve {
    ...
    private TrajSeq_swerve currentTrajectorySequence;
    ...
    public TrajSeqRun_swerve(
            TrajectoryFollower follower, PIDCoefficients headingPID_COF, VoltageSensor voltageSensor,
        ...
        turnController = new PIDFController(headingPID_COF);
    ...
    public void followTrajectorySequenceAsyncSwerve(TrajSeq_swerve trajectorySequence) {
    ...
    DriveSignal update(Pose2d poseEstimate, Pose2d poseVelocity) {
        ...
        SwerveSequenceSegment currentSegment = null;
        ...
        if (currentTrajectorySequence != null) {
            ...
            if (currentSegment instanceof TrajSeg_swerve) {
                Trajectory currentTrajectory = ((TrajSeg_swerve) currentSegment).getTrajectory();
            ...
            } else if (currentSegment instanceof TurnSegment_swerve) {
                MotionState targetState = ((TurnSegment_swerve) currentSegment).getMotionProfile().get(deltaTime);
                ...
                Pose2d startPose = currentSegment.getStartposeSwerve();
            ...
            } else if (currentSegment instanceof WaitSegment_swerve) {
                lastPoseError = new Pose2d();

                targetPose = currentSegment.getStartposeSwerve();
                driveSignal = new DriveSignal();
    ...
    private void draw(
            Canvas fieldOverlay,
            TrajSeq_swerve sequence, SwerveSequenceSegment currentSegment,
            Pose2d targetPose, Pose2d poseEstimate
    ) {
        if (sequence != null) {
            for (int i = 0; i < sequence.size(); i++) {
                SwerveSequenceSegment segment = sequence.get(i);

                if (segment instanceof TrajSeg_swerve) {
                    fieldOverlay.setStrokeWidth(1);
                    fieldOverlay.setStroke(COLOR_INACTIVE_TRAJECTORY);

                    DashboardUtil.drawSampledPath(fieldOverlay, ((TrajSeg_swerve) segment).getTrajectory().getPath());
                } else if (segment instanceof TurnSegment_swerve) {
                    Pose2d pose = segment.getStartposeSwerve();

                    fieldOverlay.setFill(COLOR_INACTIVE_TURN);
                    fieldOverlay.fillCircle(pose.getX(), pose.getY(), 2);
                } else if (segment instanceof WaitSegment_swerve) {
                    Pose2d pose = segment.getStartposeSwerve();
        ...
        if (currentSegment != null) {
            if (currentSegment instanceof TrajSeg_swerve) {
                ...
                Pose2d pose = currentSegment.getStartposeSwerve();
